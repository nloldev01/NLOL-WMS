...
export default function UnitsTable() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const endpoint = editUnit ? `/master-data/units/${editUnit.id}/` : '/master-data/units/'
        ...
        {loading ? (
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-10 text-center text-gray-400">No units found</td>
                </tr>
              ) : paginated.map((unit, idx) => (
                <tr key={unit.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-3 text-gray-400">{(page - 1) * PAGE_SIZE + idx + 1}</td>
                  <td className="px-6 py-3 font-medium text-gray-900">{unit.name}</td>
