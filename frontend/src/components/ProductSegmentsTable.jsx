...
export default function ProductSegmentsTable() {
  ...
  const fetchProductSegments = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch product segments:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  // ── Filter + Paginate ─────────────────────────────────────────────────────
  const filtered = productSegments.filter(s =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      const endpoint = editSegment
        ? `/master-data/product-segments/${editSegment.id}/`
            ...
            <tbody className="divide-y divide-gray-50">
              {paginated.length === 0 ? (
                ...
                <tr key={segment.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-3 font-medium text-gray-900">{segment.name}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Premium Juices"
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
              ...
              >
                {submitting ? 'Saving...' : editSegment ? 'Update Segment' : 'Create Segment'}
              </button>
            </div>
